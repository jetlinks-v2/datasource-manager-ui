...
export const changeStatus_api = (id: string, status: '_disable' | '_enable') =>
  request.put(`/datasource/config/${id}/${status}`)
// 新增/更新数据源
export const saveDataSource_api = (data: any) =>
  data.id ? request.patch(`datasource/config`, data) : request.post(`/datasource/config`, data)
...
export const delSaveRow_api = (datasourceId: string, table: string, data: any) =>
  request.post(`/datasource/rdb/${datasourceId}/table/${table}/drop-column`, data)
